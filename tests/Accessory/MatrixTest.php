...
class MatrixTest extends PHPUnit_Framework_TestCase
{
    ...
    /**
     * @covers \Giuseppe\LearningLibrary\Accessory\Matrix::determinant
     */
    public function testDeterminant()
    {
        $matrix = new Matrix(array(array(5, 7), array(2, -3)));
        $result = $matrix->determinant();
        $this->assertEquals($result, -29);

        $matrix = new Matrix(array(array(7, 4, 2, 0), array(6, 3, -1, 2), array(4, 6, 2, 5), array(8, 2, -7, 1)));  // this tests a series of subdeterminants (and cofactor matrices)
        $result = $matrix->determinant();
        $this->assertEquals($result, -279);

        $matrix = new Matrix(array(array(-2, 3), array(5, 0.5)));
        $result = $matrix->determinant();
        $this->assertEquals($result, -16);

        $matrix = new Matrix(array(array(2, -2, 0), array(-1, 5, 1), array(3, 4, 5)));
        $result = $matrix->determinant();
    ...
    /**
     * @covers \Giuseppe\LearningLibrary\Accessory\Matrix::scalarMultiply
     */
    public function testScalarMultiply()
    {
        $matrix = new Matrix(array(array(1, 1, 1), array(1, 1, 1), array(1, 1, 1)));
    ...
    /**
     * @covers \Giuseppe\LearningLibrary\Accessory\Matrix::multiply
     */
    public function testMultiply()
    {
        ...
        $result = $matrix->multiply(new Matrix($input));   // test multiplying by itself.
